...
export default function ProductTable({ products }: ProductTableProps) {
  ...
  const selectClass = "rounded-md border border-zinc-300 px-2 py-1 dark:border-zinc-700 dark:bg-zinc-900";
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="overflow-x-auto rounded-lg border border-zinc-200 dark:border-zinc-800">
          ...
          <tbody>
            {sorted.map((product, index) => (
              <tr key={index} className="border-t border-zinc-200 dark:border-zinc-800">
                ...
                <td className="px-3 py-2">{product.reviewCount ?? "-"}</td>
                <td className="px-3 py-2">{product.rating ?? "-"}</td>
                <td className="px-3 py-2">
                  {product.url ? (
                    <a href={product.url} target="_blank" rel="noreferrer" className="text-blue-600 underline dark:text-blue-400">
                      링크
                    </a>
                  ) : (
                    "-"
        ...
        {sorted.length === 0 && (
          <p className="px-3 py-4 text-center text-sm text-zinc-500">조건에 맞는 상품이 없습니다.</p>
        )}
      </div>
    </div>
  );
