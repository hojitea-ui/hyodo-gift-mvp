...
- 서버 왕복이 없으므로 정적 사이트로 배포하기 쉽고, 실제로 Vercel에 배포해 라이브 URL로도 접속 가능하다 (9절 참고).
- 개발 중에는 `npm run dev`로 로컬(`http://localhost:3000`)에서 확인한다.
...
| 배포 | Vercel (GitHub 연동, master 브랜치 자동 배포) | 백엔드가 없어 별도 서버 설정·환경변수 없이 GitHub 저장소만 연결하면 바로 배포됨 |
...
- **이 프로젝트는 AI 에이전트 교육 과정 과제다.** 교재에 명시된 채점 루브릭상 "실행 가능한 웹 서비스" 제출물은 GitHub 저장소 또는 배포 URL 중 하나만 있어도 형식상 충족되지만, 이번 과제는 둘 다 준비하기로 하고 **Vercel에 실제로 배포**했다.
  - (참고: 같은 과정의 다른 과제인 `backend_homework`(하루 출석체크 앱)는 실제 동작하는 공개 URL이 루브릭상 필수라 이것과 기준이 다르다. 혼동하지 않도록 구분한다.)
- **배포**: GitHub 저장소(`hojitea-ui/hyodo-gift-mvp`)를 Vercel 프로젝트에 연결해 `master` 브랜치 push마다 자동 재배포되도록 구성했다. 백엔드/DB가 없는 구조라 환경변수나 서버 설정 없이 저장소 연결만으로 바로 빌드·배포됐다.
- **로컬 실행도 계속 지원한다** — README에 아래 내용을 포함해, 배포 URL 없이도 코드만으로 확인 가능하게 유지한다:
  - 프로젝트 개요 (PRD 요약: 무엇을, 왜 만들었는지 — 옥 지압봉 단독 → 용돈박스+지압봉 → 용돈박스+결합상품으로 컨셉이 바뀐 배경도 짧게 언급)
  - 실행 방법: `npm install` → `npm run dev` → `localhost:3000` 접속
  - 샘플 데이터: 바로 업로드해볼 수 있는 예시 CSV 파일(`sample-data/competitors-sample.csv`)을 저장소에 포함해, 코드를 직접 조사하지 않아도 기능을 확인할 수 있게 한다.
  - 주요 화면 스크린샷 (업로드 화면, 대시보드 화면)
  - 폴더 구조 및 핵심 로직 위치 안내 (`lib/analysis.ts` 등 — 코드 리뷰 편의)
  - 배포된 라이브 URL 링크
- 로컬·배포 둘 다 별도 환경변수나 외부 서비스 키 없이 동일하게 동작한다.
